...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (Params().MiningRequiresPeers()) {
        if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "HelpTheHomeless is not connected!");

        if (IsInitialBlockDownload())
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HelpTheHomeless is downloading blocks...");
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED)
        ...
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "HelpTheHomeless is syncing with network...");
    ...
    devfee.push_back(Pair("amount", GetDevelopersPayment(pindexPrev->nHeight+1, pblock->vtx[0]->GetValueOut())));
